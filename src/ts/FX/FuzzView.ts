...
class FuzzView extends FXView {
    ...
    initEvent() {
        ...
        this.type.on('change', () => {
            return this.model.setParam({type: this.type.val()});
        });
        return this.gain.on('change', () => {
            return this.model.setParam({gain: parseFloat(this.gain.val())/ 100.0});
        });
    }
    ...
    setParam(p) {
        ...
        if (p.gain != null) { return this.gain.val(p.gain * 100); }
    }
}

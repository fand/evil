...
//        @select_pos = x:-1, y:-1
    ...
    resize() {
        ...
        this.canvas_tracks_on_dom.css({width: w_new + 'px', height: h_new + 'px'});
        this.canvas_tracks_hover_dom.css({width: w_new + 'px', height: h_new + 'px'});
    ...
    getPos(rect, wrapper, e, type) {
        const _x = Math.floor(((e.clientX - rect.left) + this.wrapper_mixer.scrollLeft()) / this.w);
        const _y = Math.floor((((e.clientY - rect.top)  + wrapper.scrollTop()) - this.offset_translate) / this.h);
        return {
            x: _x,
            y: _y,
            type
    ...
    getPlayPos(rect, wrapper, e) {
        const _x = Math.floor(((e.clientX - rect.left) + this.wrapper_mixer.scrollLeft()) / this.w);
        let _y = Math.floor((((e.clientY - rect.top)  + wrapper.scrollTop()) - this.offset_translate) / this.h);
    ...
    initEvent() {
        ...
        this.canvas_tracks_hover_dom.on('mousemove', e => {
            ...
            if (pos.y >= 0) {
                ...
                pos = this.getPos(this.rect_tracks, this.wrapper_tracks_sub, e, 'tracks');
            ...
            if ((this.click_pos.x === pos.x) && (this.click_pos.y === pos.y)) {
                ...
                if ((this.click_pos.x !== pos.x) || (this.click_pos.y !== pos.y)) { this.copyCell(this.click_pos, pos); }
            }
        ...
        this.canvas_master_hover_dom.on('mousemove', e => {
            ...
            if ((this.click_pos.x === pos.x) && (this.click_pos.y === pos.y)) {
                ...
                if ((this.click_pos.x !== pos.x) || (this.click_pos.y !== pos.y)) { this.copyCellMaster(this.click_pos, pos); }
            }
    ...
    readSong(song, current_cells) {
        ...
        // Draw tracks
        for (let x = 0, end = Math.max(this.song.tracks.length + 1, 8), asc = 0 <= end; asc ? x < end : x > end; asc ? x++ : x--) {
            var asc1, end1;
            var t = this.song.tracks[x];
            if (t != null) {
                if (t.type != null) { this.track_color[x] = this.color_schemes[t.type]; }
                if (t.name != null) { this.drawTrackName(x, t.name); }
            }

            for (y = 0, end1 = Math.max(this.song.length + 1, 10), asc1 = 0 <= end1; asc1 ? y < end1 : y > end1; asc1 ? y++ : y--) {
        ...
        // Draw master
        this.drawMasterName();
        for (y = 0, end2 = Math.max(this.song.length + 1, 10), asc2 = 0 <= end2; asc2 ? y < end2 : y > end2; asc2 ? y++ : y--) {
    ...
    drawScene(pos, cells) {
        ...
        for (let i = 0, end = this.current_cells.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            if (this.current_cells[i] != null) { this.drawActive(i, this.current_cells[i]); }
        }
    ...
    drawDragMaster(ctx, pos) {
        ...
        const {
            name
        } = this.song.master[this.click_pos.y];
    ...
    cueTracks(x, y) {
        if ((this.song.tracks[x] != null) && (this.song.tracks[x].patterns[y] != null)) {
            ...
            return window.setTimeout(( () => this.ctx_tracks_on.clearRect((x*this.w)+4, (y*this.h)+4, 15, 16)), 100);
        }
    }
    ...
    cueMaster(x, y) {
        if (this.song.master[y] != null) {
            ...
            return window.setTimeout(( () => this.ctx_master_on.clearRect(4, (y*this.h) + 4, 15, 16)), 100);
        }
    }
    ...
    beat(is_master, cells) {
        ...
        if (is_master) {
            ...
            return window.setTimeout(( () => this.ctx_master_on.clearRect((c[0]*this.w)+3, (c[1]*this.h)+3, 16, 15)), 100);
        } else {
            return (() => {
                const result = [];
                for (c of Array.from(cells)) {
                    this.ctx_tracks_on.drawImage(this.img_play, 36, 0, 18, 18, (c[0]*this.w) + 3, (c[1]*this.h) + 3, 16, 15);
                    result.push(window.setTimeout(( () => this.ctx_tracks_on.clearRect((c[0]*this.w)+3, (c[1]*this.h)+3, 16, 15)), 100));
                }
                return result;
            })();
        }
    ...
    showSuccess(_url, song_title, user_name) {
        ...
        this.dialog.find('.dialog-twitter').attr('href', tw_url).click( () => this.closeDialog());
        return this.dialog.find('.dialog-facebook').attr('href', fb_url).click( () => this.closeDialog());
    }
    ...
    // Copy cells by drag.
    copyCell(src, dst) {
        if ((this.song.tracks[src.x] == null)) { return; }
        if ((this.song.tracks[src.x].patterns[src.y] == null)) { return; }
    ...
    selectCell(pos) {
        if ((this.song.tracks[pos.x] == null)) { return; }
        if ((this.song.tracks[pos.x].patterns[pos.y] == null)) { return; }
    ...
    getSelectPos() { if ((this.select_pos.x !== -1) && (this.select_pos.y !== -1)) { return this.select_pos; } }
}

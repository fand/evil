...
class Player {
    ...
    setBPM(bpm) {
        ...
        for (var s of Array.from(this.synth)) { s.setDuration(this.duration); }
    ...
    setKey(key){
        ...
        for (var s of Array.from(this.synth)) { s.setKey(this.key); }
    ...
    setScale(scale) {
        ...
        for (var s of Array.from(this.synth)) { s.setScale(this.scale); }
    ...
    stop() {
        for (var s of Array.from(this.synth)) { s.stop(); }
        this.is_playing = false;
        this.view.viewStop();
        return this.time = 0;
    }
    ...
    pause() {
        for (var s of Array.from(this.synth)) { s.pause(this.time); }
    ...
    toggleLoop() { return this.session.toggleLoop(); }

    noteOn(note, force) { return this.synth_now.noteOn(note, force); }
    noteOff(force)    { return this.synth_now.noteOff(force); }
    ...
    playNext() {
        if (this.is_playing) {
            ...
            for (var s of Array.from(this.synth)) { s.playAt(this.time); }
    ...
    solo(solos) {
        let s;
        if (solos.length === 0) {
            for (s of Array.from(this.synth)) { s.demute(); }
            return;
        }
        return (() => {
            const result = [];
    ...
    readSong(song) {
        ...
        for (i = 0, end = this.song.tracks.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
        ...
        for (i = 0, end1 = this.song.tracks.length, asc1 = 0 <= end1; asc1 ? i < end1 : i > end1; asc1 ? i++ : i--) {
    ...
    resetSceneLength(l) {
        this.scene_length = 0;
        return Array.from(this.synth).map((s) =>

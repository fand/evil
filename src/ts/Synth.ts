...
class Synth {
    constructor(ctx, id, player, name) {
        ...
        this.name = name;
        this.type = 'REZ';
        if ((this.name == null)) { this.name = 'Synth #' + this.id; }
    ...
    disconnect() { return this.return.disconnect(); }

    setDuration(duration) {
        this.duration = duration;
    }
    setKey(key) { return this.core.setKey(key); }
    ...
    noteOff(force){
        if (force) { this.is_performing = false; }
    ...
    sustainNote(l, r, note) {
        ...
        for (let i = l, end = r, asc = l <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    setParam(p) {
        ...
        if (p.effects != null) { return this.setEffects(p.effects); }
    }

    mute() { return this.core.mute(); }
    demute() { return this.core.demute(); }

    // Set effects' params from the song.
    setEffects(effects_new) {
        let e;
        for (e of Array.from(this.effects)) { e.disconnect(); }
        this.effects = [];

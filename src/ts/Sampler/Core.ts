...
class SamplerCore {
    ...
    noteOn(notes) {
        ...
        if (Array.isArray(notes)) {
            ...
            return Array.from(notes).map((n) => this.samples[n[0] - 1].noteOn(n[1], time));
        }
    }
        // else
    ...
    setSample(i, name) { return this.samples[i].setSample(name); }
    ...
    setParam(p) {
        if (p.samples != null) {
            for (let i = 0, end = p.samples.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {

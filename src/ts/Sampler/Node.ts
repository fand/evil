...
class SampleNode {
    constructor(ctx, id, parent) {
        ...
        const [eq1, eq2, eq3] = Array.from([this.ctx.createBiquadFilter(), this.ctx.createBiquadFilter(), this.ctx.createBiquadFilter()]);
        [eq1.type, eq2.type, eq3.type] = Array.from(['lowshelf', 'peaking', 'highshelf']);
        [eq1.Q.value, eq2.Q.value, eq3.Q.value] = Array.from([0.6, 0.6, 0.6]);
        [eq1.frequency.value, eq2.frequency.value, eq3.frequency.value] = Array.from([350, 2000, 4000]);
        [eq1.gain.value, eq2.gain.value, eq3.gain.value] = Array.from(this.eq_gains);
        ...
        if (sample.data != null) {
            ...
            req.onload = () => {
                this.ctx.decodeAudioData(
                    ...
                    err => { console.log('Failed to fetch ' + sample.url); return console.log(err);
                 });
    ...
    connect(dst) { this.dst = dst; return this.out.connect(this.dst); }

    noteOn(gain, time) {
        if ((this.buffer == null)) { return; }
        if (this.source_old != null) { this.source_old.stop(time); }
    ...
    getTimeParam() { return [this.head, this.tail, this.speed]; }

    setEQParam(eq_gains) {
        let ref;
        this.eq_gains = eq_gains;
        return [this.eq_nodes[0].gain.value, this.eq_nodes[1].gain.value, this.eq_nodes[2].gain.value] = Array.from((ref = Array.from(this.eq_gains).map((g) => g * 0.2))), ref;
    }

    getEQParam() { return this.eq_gains; }
    ...
    getData() { return this.buffer; }

    getParam() {
        return {wave: this.name, time: this.getTimeParam(), gains: this.eq_gains, output: this.getOutputParam()};
    }

    setParam(p) {
        if (p.wave != null) { this.setSample(p.wave); }
        if (p.time != null) { this.setTimeParam(p.time[0], p.time[1], p.time[2]); }
        if (p.gains != null) { this.setEQParam(p.gains); }
        if (p.output != null) { return this.setOutputParam(p.output[0], p.output[1]); }
    }
}

...
class SidebarView {
    ...
    initEvent() {
        this.master_name.on('focus', ( () => window.keyboard.beginInput())
    ...
    showMaster(o) {
        ...
        let s = '';
        if (o.name != null) { this.master_name.val(o.name); }
        if (o.bpm != null) { s += o.bpm + ' BPM 　'; }
        if (o.key != null) { s += o.key + ' 　'; }
        if (o.scale != null) { s += o.scale; }
    ...
    setBPM(b) { return this.master_bpm.val(b); }
    setKey(k) { return this.master_key.val(k); }
    setScale(s) { return this.master_scale.val(s); }
}

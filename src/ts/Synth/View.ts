...
class SynthView {
    ...
    initCanvas() {
        ...
        for (let i = 0, end = this.cells_y, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            for (var j = 0, end1 = this.cells_x, asc1 = 0 <= end1; asc1 ? j < end1 : j > end1; asc1 ? j++ : j--) {
    ...
    initEvent() {
        ...
        this.btn_fold.on('mousedown', () => {
            if (this.is_panel_opened) {
                ...
                this.btn_fold.css({top: '0px', padding: '5px 5px 5px 5px'}).removeClass('fa-angle-up').addClass('fa-angle-down');
    ...
    addNote(pos) {
        ...
        return this.ctx_on.drawImage(this.cell,
            26, 0, 26, 26,
            pos.x * 26, pos.y * 26, 26, 26
        );
    }
    ...
    sustainNote(l, r, pos) {
        ...
        for (i = l, end = r, asc = l <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
            this.ctx_on.clearRect((i % this.cells_x) * 26, 0, 26, 1000);
        }

        for (start = l+1, i = start, end1 = r, asc1 = start <= end1; asc1 ? i < end1 : i > end1; asc1 ? i++ : i--) {
    ...
    drawPattern(time) {
        ...
        for (let i = 0, end = this.cells_x, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            var y;
    ...
    inactivate() { return this.is_active = false; }

    setSynthName(name) { return this.synth_name.val(name); }
    setPatternName(name) { this.pattern_name.val(name); return this.pattern_obj.name = name; }

...
class Noise {
    ...
    constructor(ctx: AudioContext) {
        ...
        this.node.onaudioprocess = event => {
            ...
            return __range__(0, data_L.length, false).map((i) =>
                (data_L[i] = (data_R[i] = Math.random())));
        };
    }

    connect(dst) { return this.node.connect(dst); }
    setOctave(octae) {
        this.octae = octae;
    }
    setFine(fine) {
    ...
    getParam() {
        return {shape: this.shape, octave: this.octave, interval: this.interval, fine: this.fine};
    }
...
class VCO {
    ...
    constructor(ctx: AudioContext) {
        ...
        this.setFreq();
        this.osc.start(0);
        for (let i = 0; i < 7; i++) { this.oscs[i].start(TIME_OFFSET[i]); }
    }
    ...
    setShape(shape) {
        ...
        if (this.shape === 'SUPERSAW') {
            ...
            for (o of Array.from(this.oscs)) { o.disconnect(); }
    ...
    connect(dst) {
        this.dst = dst;
        this.osc.connect(this.node);
        for (var o of Array.from(this.oscs)) { o.connect(this.node); }
...
class EG {
    ...
    getADSR() { return [this.attack, this.decay, this.sustain, this.release]; }
    ...
    getParam() { return {adsr: this.getADSR(), range: this.getRange()}; }
    setParam(p) {
...
class ResFilter {
    ...
    connect(dst)  { return this.lpf.connect(dst); }
    disconnect()  { return this.lpf.disconnect(); }
    getResonance() { return this.lpf.Q.value; }
    setQ(Q) { return this.lpf.Q.value = Q; }
    getQ()  { return this.lpf.Q.value; }
}
...
class SynthCore {
    ...
    setParam(p) {
        ...
        if (p.vcos != null) {
            let asc, end;
            for (i = 0, end = p.vcos.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
                this.vcos[i].setParam(p.vcos[i]);
            }
        }
        if (p.gains != null) {
            let asc1, end1;
            for (i = 0, end1 = p.gains.length, asc1 = 0 <= end1; asc1 ? i < end1 : i > end1; asc1 ? i++ : i--) {
    ...
    setEGParam(a, d, s, r) { return this.eg.setADSR(a, d, s, r); }
    setFEGParam(a, d, s, r) { return this.feg.setADSR(a, d, s, r); }

    setFilterParam(freq, q) {
        this.feg.setRange(80, (Math.pow(freq/1000, 2.0) * 25000) + 80);
        this.filter.setQ(q);
        if (q > 1) { return this.gain_res.gain.value = 0.1 * (q / 1000.0); }
    }
    ...
    mute() { return this.is_mute = true; }
    demute() { return this.is_mute = false; }
}

...
class Mixer {
    constructor(ctx, player) {
        ...
        this.gain_tracks = (Array.from(this.player.synth).map((s) => s.getGain()));
    ...
    drawGains() {
        // Tracks
        for (let i = 0, end = this.analysers.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    setGains(gain_tracks, gain_master) {
        ...
        for (let i = 0, end = this.gain_tracks.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    getParam() {
        return {gain_tracks: this.gain_tracks, gain_master: this.gain_master, pan_tracks: this.pan_tracks, pan_master: this.pan_master};
    }

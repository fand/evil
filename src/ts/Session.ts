...
class Session {
    ...
    nextScene(pos) {
        ...
        for (let i = 0, end = this.synth.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            if ((this.song.tracks[i].patterns[this.scene_pos] == null)) { continue; }
        ...
        if (this.song.master[this.scene_pos] != null) { this.player.readScene(this.song.master[this.scene_pos]); }
    ...
    getScene(i) { return this.song.master[i]; }
    ...
    addSynth(s, _pos) {
        ...
        const s_obj = {id: s.id, type: s.type, name: s.name, patterns, params: [], gain: 1.0, pan: 0.0};
    ...
    saveTracks() {
        return (() => {
            const result = [];
            for (let i = 0, end = this.player.synth.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    readSong(song) {
        ...
        this.scene_pos = 0;
        this.scene_length = 0;
        for (let i = 0, end = this.song.tracks.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    changeSynth(id, type, synth_new) {
        ...
        const s_params = {id, type, name: 'Synth #' + id, patterns, params: [], gain: 1.0, pan: 0.0};
        this.song.tracks[id] = s_params;
        synth_new.setPattern(patterns[this.scene_pos]);

        // Swap patterns[0] and current patterns.
        [this.song.tracks[id].patterns[0], this.song.tracks[id].patterns[this.current_cells[id]]] = Array.from([this.song.tracks[id].patterns[this.current_cells[id]], this.song.tracks[id].patterns[0]]);

...
class Sampler {
    ...
    constructor(ctx: AudioContext, id: number, player: any, name: string) {
        ...
        this.name = name;
        this.type = 'SAMPLER';
        if ((this.name == null)) { this.name = 'Sampler #' + this.id; }
    ...
    noteOff() { return this.core.noteOff(); }
    ...
    addNote(time, note, gain) {
        ...
        for (let i = 0, end = this.pattern[time].length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    setParam(p) { if (p != null) { return this.core.setParam(p); } }

(function() {
  this.t.subtest('Main Control', function() {
    this.assertEq($('#control > [name=bpm]').val(), window.player.bpm + '', 'bpm');
    this.assertEq($('#control > [name=key]').val(), window.player.key + '', 'key');
    return this.assertEq($('#control > [name=mode]').val(), window.player.scale + '', 'scale');
  });
